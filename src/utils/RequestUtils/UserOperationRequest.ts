...
import {debounceRequest, doRequest} from "../request";
...
class UserOperationRequest{
    ...
    private static doUserOperation(user:string, pwd:string, url:string):Promise<object>{
        return new Promise(async (resolve) => {
            try {
                if (!user || !pwd) {
                    resolve({Ok:false})
                }else{
                    let result:any = await debounceRequest({url,data:{user,pwd},method:'GET'})
                    if (result?.User) {
    ...
    private static doModifyOperation(token:string,changeContent:any,url:string,type:string):Promise<object>{
        return new Promise(async resolve => {
            try {
                let result:any = await debounceRequest({url,data:{token,...changeContent},method:'GET'})
                if (result?.Ok){
    ...
    static addFav(favname:string,articleid:string):Promise<object>{
